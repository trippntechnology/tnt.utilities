...
public class TokenTests
{
	...
	public void Token_Create_Sections()
	{
		...
		Assert.True(Regex.IsMatch(token, "^[a-zA-Z0-9]{4}$"));

		token = Token.Create(2, 4);
		Assert.True(Regex.IsMatch(token, "^[a-zA-Z0-9]{4}(-[a-zA-Z0-9]{4}){1}$"));

		token = Token.Create(3, 4);
		Assert.True(Regex.IsMatch(token, "^[a-zA-Z0-9]{4}(-[a-zA-Z0-9]{4}){2}$"));
	}
	...
	public void Token_Using_Custom_Chars()
	{
		...
		Assert.True(int.TryParse(token, out output));
	}
}
